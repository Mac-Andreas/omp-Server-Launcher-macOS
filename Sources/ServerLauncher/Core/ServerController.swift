...
final class ServerController: ObservableObject {
    ...
    // Matches an ISO timestamp at the START of a log entry, e.g.
    // "[2026-05-26T03:57:46+0100]". The server sometimes flushes several
    // entries in one read without newlines between them, which glued the
    // timestamps together and broke the small-timestamp rendering. Split each
    // chunk at every such timestamp so one entry = one line.
    ...
    private func ingest(_ chunk: String) {
        for raw in chunk.split(separator: "\n", omittingEmptySubsequences: false) {
            ...
            // Break a line into one entry per embedded timestamp.
            ...
            // Any text before the first timestamp is its own line.

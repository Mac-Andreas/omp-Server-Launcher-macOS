    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            SettingsCard(title: "Storage") {
                Text("Free up disk space. Cache (log snapshots + the Wine prefix) is regenerated automatically; downloads (the Wine runtime + cached installer) can be re-fetched from the Overview tab.")
                    .settingsBody()
                ...
                HStack(spacing: 12) {
                    ...
                    Button("Delete downloads") {
                        wine.uninstall()             // remove extracted runtime
                        wine.removeCachedDownload()  // remove cached archive
                        flashMsg("Downloads deleted")
                    }
